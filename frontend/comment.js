...
db.collection("comments").where("postId", "==", postId).where("parent", "==", '').orderBy('timeCreated', 'asc')
...
function displayComments(resComments, commentId) {
    ...
    idHidden.hidden = true;
    idHidden.textContent = resComments.postId;
   ...
   // userImg.setAttribute('src', resComments.userProfilePictureURL);
    ...
    const replyCont = document.createTextNode(resComments.numberOfReplies);
    ...
    replyLink.appendChild(replyCont);
