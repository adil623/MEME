    ...
    firebase.auth().currentUser.getIdToken().then((token) => {
		...
		if (ValidateEmail(email)) {  
				firebase.auth().currentUser.getIdToken().then((token) => {
					console.log('sending request to', 'https://us-central1-meme-project-0.cloudfunctions.net/webApi/api/user', 'with ID token in Authorization header.');
					const xhr = new XMLHttpRequest();
					xhr.open('GET', `https://us-central1-meme-project-0.cloudfunctions.net/webApi/api/user/${email}`, true);
					xhr.setRequestHeader('Authorization', 'Bearer ' + token);
					xhr.setRequestHeader('Content-type', 'application/json');
					xhr.onload = async () => {
						if (xhr.status == 200) {
                            ...
                            const msg = "no valid email found";
		                    windows.location.href = "index.html?error=" + msg;
...
const currentuserProfile = document.getElementById('profile');
currentuserProfile.addEventListener('click' , (e) => {
	e.preventDefault();
