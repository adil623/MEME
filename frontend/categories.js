...
function latestPost() {
    ...
    firebase.auth().currentUser.getIdToken().then((token) => {
        ...
        const xhr = new XMLHttpRequest();
        xhr.open('POST', 'https://us-central1-meme-project-0.cloudfunctions.net/webApi/api/post/hot/category', true);
        xhr.setRequestHeader('Authorization', 'Bearer ' + token);
        xhr.setRequestHeader('Content-type', 'application/json');
        xhr.onload = async () => {
            if (xhr.status == 200) {
...
function displayPost(resObject) {
    ...
    usernTime.classList.add('text-capitalize' , 'display-inline');
