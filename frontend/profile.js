...
firebase.auth().onAuthStateChanged(function (user) {
	if (user) {
		...
		window.location.href = "login.html?error=" + msg;
	}
});


//checks for error free email
...
//displaying dom objects
function domDisplay(resObject) {
	document.querySelector(".profile-img").src = resObject.imageURL;
	document.querySelector(".profile-pic").src = resObject.imageURL;
	const username = document.getElementById('user-name');
	const userEmail = document.getElementById('user-email');
	const description = document.getElementById('description');
	const location = document.getElementById('location');
	const textLocation = document.createTextNode(resObject.location)
	const fullName = document.querySelector('.fulluserName');
	const country = document.querySelector('.country');
	const gender = document.querySelector('.gender');
	const dob = document.querySelector('.dob');
	const editDescription = document.querySelector('.about');
	...
	userEmail.textContent = resObject.email;
	username.textContent = resObject.fullName;
	fullName.value = resObject.fullName;
	country.value = resObject.location;
	gender.value = resObject.gender;
	dob.value = resObject.dob;
	editDescription.value = resObject.description;
	//location.textContent = resObject.location;
...
//conversion of image into base64
let base64Image = '';
...
function postImage(base64Image) {
	...
	if (firebase.auth().currentUser) {
		...
		firebase.auth().currentUser.getIdToken().then((token) => {
			console.log('sending request to', 'https://us-central1-meme-project-0.cloudfunctions.net/webApi/api/user', 'with ID token in Authorization header.');
			const xhr = new XMLHttpRequest();
			xhr.open('POST', 'https://us-central1-meme-project-0.cloudfunctions.net/webApi/api/user/uploadImage', true);
			xhr.setRequestHeader('Authorization', 'Bearer ' + token);
			xhr.setRequestHeader('Content-type', 'application/json');
			xhr.onload = () => {
				if (xhr.status == 200) {
					const resObject = JSON.parse(xhr.responseText);
					const img = document.querySelector('.profile-pic');
					const img2 = document.querySelector(".profile-img");
					img.setAttribute('src', resObject.imageURL);
					img2.setAttribute('src', resObject.imageURL);
...
surprise.addEventListener('click', (e) => {
	...
	if (firebase.auth().currentUser) {
		...
		firebase.auth().currentUser.getIdToken().then((token) => {
			console.log('sending request to', 'https://us-central1-meme-project-0.cloudfunctions.net/webApi/api/user', 'with ID token in Authorization header.');
			const xhr = new XMLHttpRequest();
			xhr.open('POST', 'https://us-central1-meme-project-0.cloudfunctions.net/webApi/api/user/selectRandomPicture', true);
			xhr.setRequestHeader('Authorization', 'Bearer ' + token);
			xhr.setRequestHeader('Content-type', 'application/json');
			xhr.onload = () => {
				if (xhr.status == 200) {
...
editDetails.addEventListener('click', (e) => {
	...
	const sectionEdit = document.getElementById('editForm');
    const sectionPost = document.getElementById('show-post');
    const sectionSave = document.getElementById('save-post');
    const upvotesSection = document.getElementById('upvote-post');
	sectionMain.style.display = 'none';
	sectionEdit.style.display = 'block';
    sectionPost.style.display = 'none';
    sectionSave.style.display = 'none';
    upvotesSection.style.display = 'none';
	...
	detailsForm.addEventListener('submit', (e) => {
		...
		const description = document.querySelector('.about').value;
		...
		firebase.auth().currentUser.getIdToken().then((token) => {
			console.log('sending request to', 'https://us-central1-meme-project-0.cloudfunctions.net/webApi/api/user', 'with ID token in Authorization header.');
			const xhr = new XMLHttpRequest();
			xhr.open('PUT', 'https://us-central1-meme-project-0.cloudfunctions.net/webApi/api/user/', true);
			xhr.setRequestHeader('Authorization', 'Bearer ' + token);
			xhr.setRequestHeader('Content-type', 'application/json');
			xhr.onload = async () => {
				if (xhr.status == 200) {
					...
					if (msg != null) {
						...
						//	showError('please fill in all the credentials');
					}
...
SavePost.addEventListener('click' , (e) => {
    ...
    sectionPost.style.display = 'none';
    upvotesSection.style.display = 'none';
    sectionSave.style.display = 'block';
	const user = firebase.auth().currentUser
	if(user) {
		latestsave(user.email);
		window.onscroll = function (ev) {
            if ((window.innerHeight + window.pageYOffset) >= document.body.offsetHeight) {
                if (scrollTimeFunctionId == null) {
                    //alert("you're at the bottom of the page");
...
upVotes.addEventListener('click' , (e) => {
    ...
    sectionPost.style.display = 'none';
    sectionSave.style.display = 'none';
    upvotesSection.style.display = 'block';
    const user = firebase.auth().currentUser
    if(user) {
		upvotePost(user.email);
		window.onscroll = function (ev) {
            if ((window.innerHeight + window.pageYOffset) >= document.body.offsetHeight) {
                if (scrollTimeFunctionId == null) {
                    //alert("you're at the bottom of the page");
...
function upvotePost(email) {

    const params = {
		email: email,
        postId: LatestUpvotedId
    };
    const jsonObject = JSON.stringify(params);
    ...
    firebase.auth().currentUser.getIdToken().then((token) => {
        ...
        const xhr = new XMLHttpRequest();
        xhr.open('POST', 'https://us-central1-meme-project-0.cloudfunctions.net/webApi/api/post/upvoted', true);
        xhr.setRequestHeader('Authorization', 'Bearer ' + token);
        xhr.setRequestHeader('Content-type', 'application/json');
        xhr.onload = async () => {
            if (xhr.status == 200) {
                ...
                if (length > 0) {
                    LatestUpvotedId = resObject[resObject.length - 1].postId;
                   
                    for (i = 0; i < resObject.length; i++) {
                        console.log('hi');
                        displayupvoted(resObject[i].postData);
                    }
                }
            } else if (xhr.status == 400) {
...
function latestsave(email) {
    ...
    firebase.auth().currentUser.getIdToken().then((token) => {
        console.log('sending request to', 'https://us-central1-meme-project-0.cloudfunctions.net/webApi/api/post/', 'with ID token in Authorization header.');
        const xhr = new XMLHttpRequest();
        xhr.open('POST', 'https://us-central1-meme-project-0.cloudfunctions.net/webApi/api/post/saved', true);
        xhr.setRequestHeader('Authorization', 'Bearer ' + token);
        xhr.setRequestHeader('Content-type', 'application/json');
        xhr.onload = async () => {
            if (xhr.status == 200) {
                ...
                const length = resObject.length;
                if (length > 0) {
                    LatestSaveId = resObject[resObject.length - 1].postId;
                   
                    for (i = 0; i < resObject.length; i++) {
                        console.log('hi');
                        displaySave(resObject[i].postData);
                    }
                }
            } else if (xhr.status == 400) {
...
function latestPost(email) {
    ...
    firebase.auth().currentUser.getIdToken().then((token) => {
        console.log('sending request to', 'https://us-central1-meme-project-0.cloudfunctions.net/webApi/api/post/', 'with ID token in Authorization header.');
        const xhr = new XMLHttpRequest();
        xhr.open('POST', 'https://us-central1-meme-project-0.cloudfunctions.net/webApi/api/post/email', true);
        xhr.setRequestHeader('Authorization', 'Bearer ' + token);
        xhr.setRequestHeader('Content-type', 'application/json');
        xhr.onload = async () => {
            if (xhr.status == 200) {
...
function displayupvoted(resObject) {
  ...
  //console.log(resObject.userProfilePictureURL);
    ...
    const section = document.querySelector('.upvote-section');
    ...
    dropdown.classList.add("dropdown" , "d-inline" , 'float-right');
    ...
    db.collection("users").where("email", "==", resObject.email)
...
function displayPost(resObject) {
    ...
    function displaySave(resObject) {
    //console.log(resObject.userProfilePictureURL);
    // console.log(resObject.timeCreated);
    //const checkLike =  db.collection('postUpvotes').where('postId', '==' ,  resObject.id).where('email', '==', firebase.auth().currentUser.email);
    //    console.log(checkLike.doc.data());
    ...
    const dislikeCheck = () => {
        ...
        saveSection.addEventListener('click', (e) => {
            ...
            if (clickedNode.classList.contains("like-icon") && clickedNode.parentNode.nextSibling.childNodes[0].classList.contains("dislike-dark-icon")) {
                ...
                if(clickedNode.matches(".fa-tired")) {
                    ...
                    const indexNo = parseInt(indexnode);
                    const likePost = postArray[indexNo - 1];
                    let downvote = likePost.numberOfDownvotes + 1;
        ...
        const upvoteSection = document.querySelector('.upvote-section');
        upvoteSection.addEventListener('click', (e) => {
            e.preventDefault();
            //const childnodes = postSection.childNodes;
            const clickedNode = e.target;
            console.log(clickedNode)
...
const logout = document.getElementById('logout');
 ...
 const removePicture = document.getElementById('removePic');
removePicture.addEventListener('click', (e) => {
	e.preventDefault();
	if (firebase.auth().currentUser) {
		const email = firebase.auth().currentUser.email;
		const params = {
		...
		firebase.auth().currentUser.getIdToken().then((token) => {
			console.log('sending request to', 'https://us-central1-meme-project-0.cloudfunctions.net/webApi/api/user', 'with ID token in Authorization header.');
			const xhr = new XMLHttpRequest();
			xhr.open('POST', 'https://us-central1-meme-project-0.cloudfunctions.net/webApi/api/user/removePicture', true);
			xhr.setRequestHeader('Authorization', 'Bearer ' + token);
			xhr.setRequestHeader('Content-type', 'application/json');
			xhr.onload = () => {
				if (xhr.status == 200) {
