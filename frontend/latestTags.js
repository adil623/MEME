        ...
        firebase.auth().currentUser.getIdToken().then((token) => {
            ...
            xhr.onload = async () => {
                if (xhr.status == 200) {
                    ...
                    let Tagsparams = new URLSearchParams(location.search);
                    const tag = Tagsparams.get('tag');
                    document.querySelector(".profile-img").src = resObject.imageURL;
                    const categoryName = document.querySelector("#category-name");
                    categoryName.innerHTML = `${tag}`;

                    const sidelinks = document.querySelectorAll('.side-links');
                    highlightCategory(sidelinks);
...
function highlightCategory(sidelinks) {
    let Categoryparams = new URLSearchParams(location.search);
    const category = Categoryparams.get('category');
    for ( i = 0 ; i < sidelinks.length ; i++) {
        if(sidelinks[i].text == category){
...
function latestPost() {
    let Tagparams = new URLSearchParams(location.search);
    ...
    firebase.auth().currentUser.getIdToken().then((token) => {
        ...
        const xhr = new XMLHttpRequest();
        xhr.open('POST', 'https://us-central1-meme-project-0.cloudfunctions.net/webApi/api/post/latest/tag', true);
        xhr.setRequestHeader('Authorization', 'Bearer ' + token);
        xhr.setRequestHeader('Content-type', 'application/json');
        xhr.onload = async () => {
            if (xhr.status == 200) {
...
const hotTab = document.querySelector('#hot-tab');
hotTab.addEventListener('click' , (e) => {
    e.preventDefault();
    let Tagsparams = new URLSearchParams(location.search);
    const tag = Tagsparams.get('tag');
    window.location.href = "hotTags.html?tag=" + tag;

})
const latestTab = document.querySelector('#latest-tab');
latestTab.addEventListener('click' , (e) => {
    e.preventDefault();
    let Tagsparams = new URLSearchParams(location.search);
    const tag = Tagsparams.get('tag');
    window.location.href = "latestTags.html?tag=" + tag;
